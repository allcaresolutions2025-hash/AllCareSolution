...
export async function POST(req: Request) {
  ...
  if (pendings.length === 0) return NextResponse.json({ ok: true, restoredCount: 0, totalRestored: 0 });

  const totalRestored = pendings.reduce((s, p) => s + p.startBalance, 0);
  const perUser = new Map<string, number>();
  for (const p of pendings) perUser.set(p.userId, (perUser.get(p.userId) ?? 0) + p.startBalance);
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const [userId, delta] of Array.from(perUser.entries())) {
      await tx.wallet.update({ where: { userId }, data: { proMaxBalanceAvailable: { increment: delta } } });
    }
  });
