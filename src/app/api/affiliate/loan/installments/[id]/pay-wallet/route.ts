...
import { formatRupees, calcTotalPenalty, daysOverdueIst, loanWalletSurcharge, loanWalletChargePaise } from "@/lib/loan";

export const dynamic = "force-dynamic";

// Pay a loan installment using Pin Wallet points. Cost = installment + 9%
// surcharge + any accrued overdue penalty. On success the installment is marked
// paid (VERIFIED) immediately — no receipt/admin review needed — and the loan is
// closed once its final installment clears.
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  const daysOverdue = daysOverdueIst(inst.dueDate);
  const penalty = calcTotalPenalty(inst.loan.amount, daysOverdue);
  const surcharge = loanWalletSurcharge(inst.amount);
  const total = loanWalletChargePaise(inst.amount, penalty);
  ...
  if ((wallet?.pinWalletBalance ?? 0) < total) {
    return NextResponse.json(
      { error: `Not enough Pin Wallet points. This installment needs ${formatRupees(total)} (incl. 9% + penalty).` },
      { status: 400 },
    );
  }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.loanInstallment.update({
        ...
        data: {
          ...
          reviewerNotes: `Paid via Pin Wallet — ${formatRupees(inst.amount)} + ${loanWalletSurcharge(inst.amount) > 0 ? "9% (" + formatRupees(surcharge) + ")" : "0%"}${penalty > 0 ? " + penalty " + formatRupees(penalty) : ""}`,
