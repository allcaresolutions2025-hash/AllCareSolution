  ...
  async function handleClaim() {
    ...
    try {
      const res = await fetch("/api/affiliate/rewards", {
        ...
        body: JSON.stringify({ level: FORTY_COMBO_LEVEL }),
      });
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-[10px] font-semibold uppercase tracking-wide text-violet-700 flex items-center gap-1">
              <Sparkles className="h-3 w-3" /> 2000-pt member reward
            </div>
        ...
        <div className="shrink-0">
          {claim ? (
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              >
                {loading ? "Submitting…" : (<><Gift className="h-4 w-4" /> Request 40 Combo <ChevronRight className="h-3.5 w-3.5" /></>)}
              </button>
            </div>
          )}
        </div>
