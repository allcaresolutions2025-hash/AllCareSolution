...
import { Gift, CheckCircle2, Clock, Truck, Package, XCircle, ChevronRight, Sparkles } from "lucide-react";
...
export function WelcomeKitCard({ claim }: { claim: ClaimInfo }) {
  ...
  return (
    <div className="card p-5 border-2 border-amber-300 bg-gradient-to-br from-amber-50/60 to-white">
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-[10px] font-semibold uppercase tracking-wide text-amber-700 flex items-center gap-1">
              <Sparkles className="h-3 w-3" /> Joining gift
            </div>
            <div className="font-bold text-base leading-tight mt-0.5">Welcome Kit</div>
            <p className="text-xs text-muted-foreground mt-1 max-w-md">
              Every new member is eligible for the Welcome Kit. Apply once — no leg-count requirement.
            </p>
          </div>
        </div>
        ...
        <div className="shrink-0">
          {claim ? (
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              >
                {loading ? "Submitting…" : (<><Gift className="h-4 w-4" /> Apply for Welcome Kit <ChevronRight className="h-3.5 w-3.5" /></>)}
              </button>
            </div>
          )}
        </div>
