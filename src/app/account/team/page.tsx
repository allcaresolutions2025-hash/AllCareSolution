  ...
  return (
    <div className="space-y-6">
      ...
      {snapshot.totalMembers === 0 ? (
        ...
        <>
          <LevelBreakdown summary={snapshot.summary} />
...
function LevelBreakdown({ summary }: { summary: ReturnType<typeof import("@/lib/network").summarizeByLevel> }) {
  return (
    <div className="card p-0 overflow-hidden">
      <div className="px-5 py-3 border-b">
        <h2 className="font-semibold">Levels at a glance</h2>
        <p className="text-xs text-muted-foreground">Each level shows team size and the product sales generated by people on that level.</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-xs uppercase text-muted-foreground">
            <tr>
              ...
              <th className="text-right px-5 py-2 font-medium">Your commission</th>
            </tr>
          </thead>
          <tbody>
            {summary.map((s) => (
              <tr key={s.level} className="border-t">
                <td className="px-5 py-2 font-medium">L{s.level}</td>
                <td className="text-right px-5 py-2">{s.members}</td>
                <td className="text-right px-5 py-2">{s.activeMembers}</td>
                <td className="text-right px-5 py-2">{formatINR(s.teamSalesPaise)}</td>
                <td className="text-right px-5 py-2 text-brand-700">{formatINR(s.commissionEarnedPaise)}</td>
              </tr>
            ))}
          </tbody>
        </table>
