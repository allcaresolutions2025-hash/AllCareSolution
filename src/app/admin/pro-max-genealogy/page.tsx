...
// Admin view of the PIN PRO MAX trees. Unlike the 1000-pt genealogy (one system
// leader), Pro Max can have many independent trees — one per member who started
// their own by getting a Pro Max pin approved (isProMax + proMaxReferrerId null).
// Pick a root to view its tree.
export default async function AdminProMaxGenealogyPage({
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="card p-10 text-center">
          <Crown className="h-10 w-10 text-amber-300 mx-auto mb-2" />
          <p className="text-sm text-muted-foreground">
            No Pro Max trees yet. A tree appears here once a member&apos;s Pro Max pin request is
            approved and they start placing Pro Max members.
          </p>
        </div>
      </div>
    );
  ...
  const totalMembers = snapshot.totalMembers + 1; // include the root
  ...
  // Per-node Pro Max balances for the tree cards.
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          The separate Pin Pro Max binary trees. Pick a root below; use the corner controls to zoom or go fullscreen.
        </p>
      </div>

      {/* Root selector — one entry per independent Pro Max tree */}
      {roots.length > 1 && (
      ...
      <div className="card p-5 bg-gradient-to-br from-amber-500 to-amber-700 text-white">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="text-right">
            ...
            <div className="text-xs text-amber-100 mt-1">L {selected.proMaxLeftLegCount} · R {selected.proMaxRightLegCount}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        <Kpi icon={<Crown className="h-5 w-5" />} label="Downline" value={String(snapshot.totalMembers)} sub="below root" />
        <Kpi icon={<Layers className="h-5 w-5" />} label="Tree depth" value={`${maxDepth} ${maxDepth === 1 ? "level" : "levels"}`} sub="deepest path" />
      </div>

      {/* The expandable tree */}
      <BinaryTreeZoomable people={treePeople} addMemberPath="/affiliate/dashboard/add-member/pro-max" />
    </div>
  );
}
