      ...
      include: {
        loan: {
          select: {
            id: true,
            amount: true,
            user: { select: { name: true, referralCode: true, phone: true, whatsappNumber: true } },
          },
        },
      },
    }),
    ...
    <div className="space-y-6">
      ...
      <section className="card p-5">
        <h2 className="font-semibold flex items-center gap-2 mb-4">
          <AlertTriangle className="h-4 w-4 text-red-600" /> Overdue ({overdue.length})
        </h2>
        {overdue.length === 0 ? (
          <p className="text-sm text-muted-foreground">Nobody in your team is overdue. Nice work.</p>
        ) : (
      ...
      <section className="card p-5">
        <h2 className="font-semibold flex items-center gap-2 mb-4">
          <CalendarDays className="h-4 w-4 text-amber-600" /> Due in the next 7 days ({dueSoon.length})
        </h2>
        {dueSoon.length === 0 ? (
          <p className="text-sm text-muted-foreground">Nothing falling due this week.</p>
        ) : (
